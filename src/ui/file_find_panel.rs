...
use crate::model::FileFind;
use crate::theme;
use crate::ui::input_box;

pub fn render(frame: &mut Frame, area: Rect, state: &FileFind) {
    ...
    let match_label = if state.query.text.is_empty() {
        ...
        format!(" {} files  searching\u{2026}  ", state.results.len())
    };
...
fn render_results(frame: &mut Frame, area: Rect, state: &FileFind) {
    let items: Vec<ListItem> = state
        .results
        .iter()
        .map(|r| {
            let label = r.rel_path.display().to_string();
